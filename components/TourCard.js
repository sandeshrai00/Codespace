  ...
  return (
    <Link href={`/${lang}/tours/${tour.id}`} className="block group">
      <div className="bg-white rounded-2xl shadow-card hover:shadow-glass-lg transition-all duration-500 h-full flex flex-col overflow-hidden border border-gray-100 group-hover:scale-[1.02] group-hover:border-primary-200">
        ...
        <div className="relative h-32 sm:h-48 md:h-56 w-full overflow-hidden hover-zoom">
          {tour.banner_image ? (
            <Image
        ...
        {/* Card Content */}
        <div className="p-2 sm:p-5 md:p-6 flex-1 flex flex-col">
          <h3 className="text-sm sm:text-xl font-bold text-gray-900 mb-1 sm:mb-3 group-hover:text-primary-600 transition-colors line-clamp-2">
            {localizedTitle}
          </h3>
          
          {/* Location with icon */}
          <div className="flex items-center gap-1 text-gray-600 text-[10px] sm:text-sm mb-1 sm:mb-3">
          ...
          {/* Duration */}
          <div className="flex items-center gap-4 mb-2 sm:mb-4 text-[10px] sm:text-sm text-gray-600">
            <div className="flex items-center gap-1">
              <svg className="w-3 h-3 sm:w-4 sm:h-4 text-secondary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between gap-3 mt-auto pt-4 border-t border-gray-100">
            <div>
              <div className="text-[8px] sm:text-xs text-gray-500 mb-1">From</div>
